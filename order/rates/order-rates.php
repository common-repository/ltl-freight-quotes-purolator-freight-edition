 ...
 */
if (!class_exists('EnPurolatorLtlOrderRates')) {

    class EnPurolatorLtlOrderRates
    {
        ...
        public function en_order_before_calculate_totals($and_taxes, $order)
        {
            ...
            if (isset($woocommerce->cart)) {
                ...
                // Shipping address
                if (!empty($order_id) && !empty($order_status) && $order_status != 'checkout-draft' && method_exists($order, 'get_address')) {
                    ...
                    foreach ($shipping as $item_id => $per_shipment) {
                        ...
                        switch ($method_id) {
                            case 'purolator_ltl':
                                ...
                                if (empty($errors)) {
                                    $shipping_class = new Purolator_LTL_Freight_Shipping();
                ...
                if ($order_recreated) {
                    ...
                    $purolator_ltl_curl_obj = new Purolator_Feight_Curl_Request();
                    $purolator_ltl_curl_obj->purolator_freight_get_curl_response(PUROLATOR_FREIGHT_FDO_HITTING_URL, $request_data);
                }
            }
        }
    }

    new EnPurolatorLtlOrderRates();
}