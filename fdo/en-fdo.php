...
class EnPurolatorFdo
{
    ...
    public function en_cart_package($package)
    {
        ...
        $this->en_fdo_meta_data['plugin_name'] = 'purolator_quotes';
        $accessorials['residential'] = get_option('purolator_ltl_always_include_residential') == 'yes' ? true : false;
        $accessorials['liftgate'] = get_option('purolator_ltl_always_tailgate') == 'yes' ? true : false;
    ...
    public function en_package_items($items)
    {
        ...
        foreach ($items as $item_key => $item_data) {
            $nmfc_number = $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $variantId = $markup = '';
            $attributes = [];
            extract($item_data);
